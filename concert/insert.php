...
<?
    ...
    */
	...
	$regist_day = date("Y-m-d (H:i)");  // 현재의 '년-월-일-시-분'을 저장
		/*   단일 파일 업로드 
		$upfile_name	 = $_FILES["upfile"]["name"]; //진짜 파일이름 빼옴
		$upfile_tmp_name = $_FILES["upfile"]["tmp_name"]; //임시파일...원본에 손상을 안주기위해
	...
	for ($i=0; $i<$count; $i++)
	{
		...
		if (!$upfile_error[$i]) //에러가 발생되지 않으면
		{
			...
			if ( ($upfile_type[$i] != "image/gif") &&
				...
				($upfile_type[$i] != "image/pjpeg") ) //익스플로어때문에 이거 달아야함
			{
				echo("
					<script>
	...
	if ($mode=="modify")
	{
		...
		for ($i=0; $i<$count; $i++)					// update DB with the value of file input box
		{
			...
			if ($del_ok[$i] == "y")
			{
				...
				unlink($delete_path); //업로드된 파일 삭제..매우 중요!폴더안의 파일까지 삭제해준다
